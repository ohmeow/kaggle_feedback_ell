...
from .configuration_cocolm import COCOLMConfig
from .convert_state_dict import get_checkpoint_from_transformer_cache
        ...
        if model_type is not None and "state_dict" not in kwargs:
            if model_type in cls.supported_convert_pretrained_model_archive_map:
                ...
                if pretrained_model_name_or_path in pretrained_model_archive_map:
                    ...
                    raise RuntimeError("No pre-trained checkpoint !")
        ...
        if _k in state_dict and "config" in kwargs:
            ...
            if config.max_position_embeddings > state_dict[_k].shape[0]:
                ...
                new_postion_embedding = state_dict[_k].data.new_tensor(
                    torch.ones(size=(config.max_position_embeddings, state_dict[_k].shape[1])), dtype=torch.float
                )
...
class COCOLMEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.layer_norm_type in ("post",):
            self.LayerNorm = COCOLMLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class SelfMultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        q = q.contiguous().view(tgt_len, bsz * self.num_heads, self.qk_head_dim).transpose(0, 1) * self.scaling
        if k is not None:
            k = k.contiguous().view(-1, bsz * self.num_heads, self.qk_head_dim).transpose(0, 1)
        if v is not None:
        ...
        if need_weights:
            ret_attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len).transpose(1, 0)
...
class COCOLMAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.self_attn = SelfMultiheadAttention(config.hidden_size, config.num_attention_heads, dropout=config.attention_probs_dropout_prob)
    ...
    def forward(self, x, attention_mask, attn_bias):
        ...
        x, attn = self.self_attn(
            ...
            need_weights=True if self.config.output_attentions else False,
            attn_mask=None,
            attn_bias=attn_bias,
        )
        x[x != x] = 0
...
class COCOLMLayer(nn.Module):
    def __init__(self, config):
        ...
        if hasattr(config, "num_ffn_layers") and config.num_ffn_layers > 1:
            ...
            self.intermediate = nn.ModuleList([COCOLMIntermediate(config) for _ in range(config.num_ffn_layers)])
            self.output = nn.ModuleList([COCOLMOutput(config) for _ in range(config.num_ffn_layers)])
        else:
        ...
        if self.layer_norm_type in ("pre", "hybrid"):
            if self.num_ffn_layers > 0:
                self.LayerNorm = nn.ModuleList([COCOLMLayerNorm(config.hidden_size, eps=1e-5) for i in range(self.num_ffn_layers)])
            else:
...
class COCOLMEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList([COCOLMLayer(config) for _ in range(config.num_hidden_layers)])
        self.layer_norm_type = config.layer_norm_type
        if self.layer_norm_type in ("pre", "hybrid"):
            self.LayerNorm = COCOLMLayerNorm(config.hidden_size, eps=1e-5)
    ...
    def forward(self, hidden_states, attention_mask=None, split_lengths=None, rel_pos=None):
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_outputs = layer_module(hidden_states, attention_mask, split_lengths=split_lengths, rel_pos=rel_pos)
...
def relative_position_bucket(relative_position, num_buckets=32, max_distance=128):
    ...
    val_if_large = (
        max_exact + torch.ceil(torch.log(n.float() / max_exact) / math.log((max_distance - 1) / max_exact) * (max_bucket_val)).long()
    )
...
class COCOLMModel(COCOLMPreTrainedModel):
    def __init__(self, config):
        ...
        if self.config.rel_pos_bins > 0:
            ...
            self.rp_bucket = relative_position_bucket(
                relative_position, num_buckets=self.config.rel_pos_bins, max_distance=self.config.max_rel_pos
            )
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, inputs_embeds=None, split_lengths=None):
        ...
        if rel_pos_bias is not None and extended_attention_mask is not None:
            ...
            rel_pos_bias.masked_fill_(extended_attention_mask.unsqueeze(1).unsqueeze(2), float("-inf"))
            rel_pos_bias = rel_pos_bias.view(-1, seq_len, seq_len)
            extended_attention_mask = None

        encoder_outputs = self.encoder(
            embedding_output, attention_mask=extended_attention_mask, split_lengths=split_lengths, rel_pos=rel_pos_bias
        )
